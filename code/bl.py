...
one_hot_feature = ['carrier', 'LBS', 'age', 'consumptionAbility', 'education', 'gender', 'house', 'advertiserId',
                   'campaignId', 'creativeId',
                   'adCategoryId', 'productId', 'productType']
not_clear_feature = ['marriageStatus', 'os', 'ct', ]
vector_feature = ['appIdAction', 'appIdInstall', 'interest1', 'interest2', 'interest3', 'interest4', 'interest5', 'kw1',
                  'kw2', 'kw3', 'topic1', 'topic2', 'topic3']
#.拼接用户特征，广告特征
def pre_merge():
    ad_feature=pd.read_csv('../data/origin/adFeature.csv')
    if os.path.exists('../data/origin/userFeature.csv'):
        user_feature=pd.read_csv('../data/origin/userFeature.csv')
    else:
        userFeature_data = []
...
def pre_merge0():
    ...
    del ad_feature
    gc.collect()
    for feature in user_one_feature:
        one_feature = pd.read_csv('../data/slipt_feature/' + feature +'.csv')
        train = pd.merge(train, one_feature, how="left", on="uid")
    #here memorry error
    return train
...
def coder(data):
    ...
    train, test1, train_y, test1_y = train_test_split(train, train_y, test_size=0.3, random_state=2018)
...
if __name__ =='__main__':
    data = pre_merge()
    train_x, train_y, test_x, res = coder(data)
